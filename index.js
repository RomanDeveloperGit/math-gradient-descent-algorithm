...
// И снова проверим функцию потерь, насколько она теперь убавится после апдейта весов:
calculateValues();
checkActualState();

// Изначально функция потерь выдавала: 0.3727791772707186
// После апдейта весов: 0.37054603836295485

// Стоит отметить, что подобным образом не обучают сети, это я тут балуюсь.
// Вообще надо делить выборку на обучающую, тестовую, валидационную, прогонять эпохи, делить на батчи, да и ...
// ... сам алгоритм - зависит от ситуации. Это здесь стандартный градиент, хотя часто предпочитают ...
// ... стохастический или другие существующие варианты оптимизации. Но в ML я не так сильно погружен.
// Ну и, соответственно, веса "двигаем" после эпохи на "средний" градиент
